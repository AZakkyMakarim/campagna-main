        ...
        <tbody class="divide-y divide-gray-200">
            @foreach($finisheds as $key => $finished)
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-4 py-3">{{ $key + 1 + (((request('finished_page') ?? 1) - 1) * 10) }}</td>
                    ...
                    <td class="px-4 py-3">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" class="sr-only peer" {{ $finished->is_active ? 'checked' : '' }}
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-2">
                            <button type="button" data-route="{{ route('management.ingredient.update', $finished) }}"
...
<div x-data="editFinishedModal()" x-show="open" @open-edit-finished.window="fill($event.detail)" x-transition x-cloak
