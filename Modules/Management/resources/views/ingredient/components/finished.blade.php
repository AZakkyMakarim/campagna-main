<div class="overflow-hidden rounded-lg shadow-lg border border-gray-200 bg-white">
    <table class="w-full text-sm text-left">
        ...
        @foreach($finisheds as $key => $finished)
            <tr class="hover:bg-gray-50 transition">
                <td class="px-4 py-3">{{ $key + 1 + (((request('page') ?? 1) - 1) * 15) }}</td>
                <td class="px-4 py-3 text-nowrap">{{ $finished->name }}</td>
                <td class="px-4 py-3 text-nowrap">{{ $finished->baseUnit->name }}</td>
                <td class="px-4 py-3 text-nowrap">{{ $finished->min_stock }}</td>
                <td class="px-4 py-3">
                    <label class="inline-flex items-center cursor-pointer">
                        <input
                            type="checkbox"
                            class="sr-only peer"
                            {{ $finished->is_active ? 'checked' : '' }}
                            onchange="toggleOutletStatus(this, '{{ route('management.ingredient.update', $finished) }}')"
                        >
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center justify-center gap-2">
                        <button
                            type="button"
                            data-route="{{ route('management.ingredient.update', $finished) }}"
                            @click="$dispatch('open-edit-finished', {
...
<x-modal id="modal-form-finished" title="Tambah Bahan Jadi" size="md">
...
<div
    x-data="editFinishedModal()"
    x-show="open"
    @open-edit-finished.window="fill($event.detail)"
    x-transition
    x-cloak
            ...
            <form :action="action" method="POST">
                ...
                <div class="p-5 text-gray-300">
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Stok Min</label>
                            <input type="number" x-model="form.min_stock" step="0.1" name="min_stock" required placeholder="Masukkan Stok Minimal" class="w-full text-gray-700 px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:ring-offset-white">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">
                                Satuan
        ...
        function editFinishedModal() {
            return {
                ...
                fill(payload) {
                    ...
                    this.form = {
                        name: finished.name,
                        min_stock: finished.min_stock,
                        base_unit_id: finished.base_unit_id,
                        outlet_id: finished.outlet_id,
                    }
                }
            }
        }
