...
class ReservationController extends Controller
{
    ...
    public function preorder(Reservation $reservation){
        ...
        return view('transaction::reservation.preorder', compact('reservation', 'menus', 'categories', 'outlet', 'taxes'));
    }
    ...
    public function storePreorder(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->items as $item) {
                ...
                $doneQty = 0;
                if (!in_array($menu->category, ['makanan', 'wedangan', 'minuman', 'jede sate', 'jede bakmi', 'paket nasi'])){
                    $doneQty = $item['qty'];
                }
                OrderItem::create([
                    'order_id'      => $order->id,
...
//        ]);
    ...
    public function confirm(Reservation $reservation){
        ...
        broadcast(new OrderCreated($reservation->order->load('items')));

        $this->printOrderKitchen($reservation->order);

        toast('Customer datang!');
        return back();
    }
    ...
    public function pay(Request $request)
    {
        ...
        try {
            ...
            $this->printOrderKitchen($reservation->order, ['cashier']);
    ...
    private function printOrderKitchen($order, $roles = [])
    {
        ...
        $printers = Printer::where('outlet_id', active_outlet_id())
            ->when(count($roles) > 0, function ($query) use ($roles){
